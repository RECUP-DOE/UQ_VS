...
def train_vi_one_epoch(
    ...
    model = vi_model.base_model
    
    vi_model.train()
    n_batches = 0
    n_samples = len(data_loader.dataset)
    loss_stats = {'total_loss':0.0,'base_loss':0.0, 'vi_kl_loss': 0.0}
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        kl_loss = vi_model.compute_kl()/n_samples
        # print(kl_loss)
        total_loss = loss + temperature*kl_loss
        # print(total_loss)
        #print(f'batch:{idx}, jtvae loss: {base_loss}, jtvae rec: {base_rec_loss}, jtvae kl: {base_kl_loss}, vi kl: {kl_loss}')
        optimizer.zero_grad()
        
        total_loss.backward()
        
        #torch.nn.utils.clip_grad_norm_(vi_model.parameters(), 2.0)
        optimizer.step()
        
        # raise ValueError('A very specific bad thing happened.')
        
        loss_stats['total_loss'] += total_loss.item()
        loss_stats['base_loss'] += loss.item()
        loss_stats['vi_kl_loss'] += kl_loss.item()
        n_batches += 1

    return {x: loss_stats[x]/n_batches for x in loss_stats.keys()}



def one_gradient_sample(
    model: MoleculeModel,
    batch,
    loss_func: Callable,
    optimizer: Optimizer,
...
def eval_model(
    ...
    return {x: loss_stats[x]/n_batches for x in loss_stats.keys()}, all_preds